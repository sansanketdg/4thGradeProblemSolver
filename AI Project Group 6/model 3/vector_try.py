...
for i in range(len(lines)):
    ...
    # print(X, x_final, x_diff, x_temp)

    '''if not x_temp == 0:
        # print("x_tmp", tmp2, tmp)
        if not X == 0 and x_final == 0:
            x_final = x_temp
        elif not x_final == 0 and X == 0:
            X = x_temp
        else:
            count += 1
            # print("else", count)
    ...
    if not x_diff == 0:
        if not X == 0:
            ...
            answer = abs(x_final - x_diff)
    else:
        answer = abs(X - x_final)'''

    answer = abs(X - x_final + x_diff + x_temp)
